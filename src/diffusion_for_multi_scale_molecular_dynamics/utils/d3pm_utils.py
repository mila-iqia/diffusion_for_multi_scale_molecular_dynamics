...
def compute_q_xt_given_xo(
    one_hot_x0: torch.Tensor, q_bar_t: torch.Tensor
) -> torch.Tensor:
    """Compute q(x_t | x_0).
...
def compute_q_xt_given_xtm1(
    one_hot_xt: torch.Tensor, q_t: torch.Tensor
) -> torch.Tensor:
    ...
    return einops.einsum(
        one_hot_xt.to(q_t), torch.transpose(q_t, -2, -1), "... j, ... i j -> ... i"
    )
