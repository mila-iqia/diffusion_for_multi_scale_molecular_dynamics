...
from diffusion_for_multi_scale_molecular_dynamics.utils.basis_transformations import get_positions_from_coordinates, \
    map_lattice_parameters_to_unit_cell_vectors, get_relative_coordinates_from_cartesian_positions, \
    get_reciprocal_basis_vectors, map_numpy_unit_cell_to_lattice_parameters
...
class BaseSampleMakerArguments:
    ...
    def __post_init__(self):
        ...
        if self.sample_box_strategy == "fixed":
            ...
            self.new_box_lattice_parameters =  map_numpy_unit_cell_to_lattice_parameters(unit_cell)


class BaseSampleMaker(ABC):
    """Base class for the method making new samples."""

    def __init__(self, sample_maker_arguments: BaseSampleMakerArguments, device: str = "cpu"):
...
class BaseExciseSampleMaker(BaseSampleMaker):
    ...
    def embed_structure_in_new_box(
        ...
        new_box_center_point_reduced_coordinates = torch.ones(1, spatial_dimension) * 0.5
        ...
        new_box_atoms_cartesian_positions = cartesian_positions_as_vector_translation + new_box_center_point_cartesian_positions.numpy()
        ...
        for d in range(spatial_dimension):
            assert (torch.max(new_box_atoms_cartesian_positions[:,d]) < new_box_size[d]) and (
        ...
        reciprocal_basis_vectors = get_reciprocal_basis_vectors(
            new_basis_vectors
        )
        new_box_atoms_relative_coordinates = get_relative_coordinates_from_cartesian_positions(
        ...
        for constrained_environment in constrained_environments_after_excision:
            if self.sample_box_strategy == "fixed":
                constrained_environment_in_new_box = self.embed_structure_in_new_box(
                    ...
                    self.arguments.new_box_lattice_parameters
                )
