...
def main():
    ...
    parser.add_argument(
        "--prediction_file",
        help="MTP prediction files. Should contain position and MaxVol gamma.",
    )
    parser.add_argument(
        "--lammps_output",
        help="LAMMPS output file. Should contain the bounding box information.",
    )
    parser.add_argument(
        "--output_name", help="Name of the output file that can be loaded by OVITO. "
    )
    args = parser.parse_args()

    assert os.path.exists(
        args.lammps_output
    ), f"LAMMPS out file {args.lammps_output} does not exist."

    lattice = get_lattice_from_lammps(args.lammps_output)

    assert os.path.exists(
        args.prediction_file
    ), f"Provided prediction file {args.prediction_file} does not exist."
...
def get_lattice_from_lammps(lammps_output_file: str) -> np.ndarray:
    ...
    with open(lammps_output_file, "r") as f:
        ...
        for (
            ...
            l_yaml
        ):  # loop over LAMMPS outputs to get the MD box - we only need the first step
...
def mtp_predictions_to_ovito(pred_file: str, lattice: np.ndarray, output_name: str):
    ...
    for struct in sorted(df["structure_index"].unique()):  # iterate over LAMMPS steps
        ...
        gamma_values = df.loc[
            df["structure_index"] == struct, "nbh_grades"
        ].to_numpy()  # nbh_grade
