...
from crystal_diffusion.generators.ode_position_generator import ExplodingVarianceODEPositionGenerator, \
    ODESamplingParameters
from crystal_diffusion.generators.predictor_corrector_position_generator import PredictorCorrectorSamplingParameters
from crystal_diffusion.generators.sde_position_generator import ExplodingVarianceSDEPositionGenerator, \
...
class ForcedStartingPointLangevinGenerator(LangevinGenerator):
    ...
    def initialize(self, number_of_samples: int):
        ...
        relative_coordinates = einops.repeat(self._starting_relative_coordinates,
...
base_sampling_parameters_dict = dict(number_of_atoms=number_of_atoms,
                                     spatial_dimension=spatial_dimension,
                                     cell_dimensions=[acell, acell, acell],
                                     number_of_samples=number_of_samples)

ode_sampling_parameters = ODESamplingParameters(absolute_solver_tolerance=1.0e-5,
                                                relative_solver_tolerance=1.0e-5,
                                                **base_sampling_parameters_dict)

# Fiddling with SDE is PITA. Also, is there a bug in there?
sde_sampling_parameters = SDESamplingParameters(adaptative=False, **base_sampling_parameters_dict)


langevin_sampling_parameters = PredictorCorrectorSamplingParameters(number_of_corrector_steps=number_of_corrector_steps,
                                                                    **base_sampling_parameters_dict)
...
if __name__ == "__main__":
    ...
    ode_samples = ode_generator.sample(number_of_samples=number_of_samples, device=device, unit_cell=unit_cells)
    sde_samples = sde_generator.sample(number_of_samples=number_of_samples, device=device, unit_cell=unit_cells)

    langevin_samples = langevin_generator.sample(number_of_samples=number_of_samples,
                                                 device=device,
                                                 unit_cell=unit_cells)

    energy_calculator = EnergyCalculator(unit_cell=basis_vectors, number_of_atoms=number_of_atoms)
    ...
    langevin_label = (f"LANGEVIN (time steps = {total_time_steps}, "
    ...
    for ax in [ax1, ax2]:
        ...
        ax.fill_between([0, 100], y1=-34.6, y2=-34.1, color='yellow', alpha=0.25, label='Training Energy Range')
