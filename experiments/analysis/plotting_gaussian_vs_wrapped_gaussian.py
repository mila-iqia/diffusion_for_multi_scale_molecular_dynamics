...
if __name__ == '__main__':
    ...
    for fig, sigma, color in zip(list_figs, list_sigmas, list_colors):
        ...
        ax.plot(x, wrapped_gaussians, color=color, lw=2, ls='--', label='Wrapped Gaussian')

        ax.vlines(np.arange(imin, imax + 1), 0, ymax, color='k')

        ax.fill_betweenx([0, ymax], x1=0, x2=1, color='grey', alpha=0.25, label='Unit Cell')
        ...
        fig.tight_layout()
        fig.savefig(ANALYSIS_RESULTS_DIR.joinpath(f"Gaussian_vs_Wrapped_Gaussian_sigma={sigma}.png"))
