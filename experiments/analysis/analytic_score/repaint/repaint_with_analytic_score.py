...
if __name__ == "__main__":
    ...
    equilibrium_relative_coordinates = get_silicon_supercell(
    ...
    inverse_covariance = (
        torch.diag(torch.ones(number_of_atoms * spatial_dimension)) / variance_parameter
    ).to(device)
    ...
    sampled_harmonic_energies = get_samples_harmonic_energy(
        equilibrium_relative_coordinates, inverse_covariance, samples
    )
    ...
    equilibrium_structure = create_structure(
        lattice_basis_vectors, relative_coordinates, species
    )
    equilibrium_structure.to_file(str(repaint_dir / "equilibrium_positions.cif"))

    relative_coordinates[: len(constrained_relative_coordinates)] = (
        constrained_relative_coordinates
    )
    forced_structure = create_structure(
        lattice_basis_vectors, relative_coordinates, species
    )
