...
if __name__ == "__main__":
    ...
    samples = position_generator.sample(
        number_of_samples=number_of_samples, device=device, unit_cell=unit_cell
    ).detach()
    ...
    inverse_covariance = (torch.diag(torch.ones(nd)) / variance_parameter).to(
    ...
    fig5.savefig(
        ANALYTIC_SCORE_RESULTS_DIR
        / f"harmonic_energy_samples_{sampling_algorithm}_{number_of_atoms}_atoms.png"
    )
    plt.close(fig5)
