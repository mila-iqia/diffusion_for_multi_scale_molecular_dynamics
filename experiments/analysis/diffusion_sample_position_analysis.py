...
trajectory_top_output_directory = (
    TOP_DIR / "experiments/diffusion_mace_harmonic_data/output/"
)

trajectory_data_directory = (
    trajectory_top_output_directory / experiment_name / "diffusion_position_samples"
)
energy_sample_directory = (
    trajectory_top_output_directory / experiment_name / "energy_samples"
)
...
if __name__ == "__main__":
    ...
    pickle_path = (
        trajectory_data_directory / f"diffusion_position_sample_epoch={epoch}.pt"
    )
    ...
    default_orientation = torch.tensor([1.0, 1.0, 1.0], dtype=float_datatype) / np.sqrt(
        3.0
    )
    for idx in list_indices:
        ...
        normalized_angle = torch.arccos(
            displacements @ default_orientation / torch.sqrt(delta_squared)
        ) / (np.pi)
    ...
    ax2.vlines(
        x=torch.arccos(torch.tensor([-1, -1 / 3, 1 / 3, 1])) / np.pi,
        ymin=0,
        ymax=180,
        linestyle="--",
        color="black",
        label="Equilibrium Angles",
    )
