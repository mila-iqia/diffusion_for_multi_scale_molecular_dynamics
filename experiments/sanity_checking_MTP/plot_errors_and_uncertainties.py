...
from src.diffusion_for_multi_scale_molecular_dynamics.analysis import (
...
level = 6
exp_dir = Path(
    "/Users/brunorousseau/courtois/podman/mtp_and_artn/mount/sanity_checking_MTP/mtp_experiments/"
)
ground_truth_data_directory = exp_dir / "ground_truth_data"
mtp_experiment_path = exp_dir / f"level_{level}"
...
if __name__ == "__main__":
    ...
    list_round_indices = [1, 2, 3, 4, 5, 6]
    ...
    for round_index in list_round_indices:
        ...
        fig.suptitle(
            f"Comparing MTP Level {level} Errors to Uncertainties\nBefore and After for Round {round_index}"
        )
        ...
        for color, label in zip(["red", "green"], ["before", "after"]):
            output_file_path = str(
                mtp_experiment_path / f"eval_{round_index}_{label}.cfg.0"
            )
            nbh_file_path = str(
                mtp_experiment_path / f"eval_{round_index}_nbh_grade_{label}.cfg.0"
            )
            ...
            position_error = np.linalg.norm(
                get_positions(gt_structure) - get_positions(pred_structure)
            )
            ...
            ax1.plot(
                gamma_uncertainties[uncertain_indices],
                force_normed_errors[uncertain_indices],
                "*",
                color=color,
                ms=15,
                alpha=1.0,
                label=f"{label} Most Uncertain",
            )
