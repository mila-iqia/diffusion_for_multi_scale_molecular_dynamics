...
if __name__ == "__main__":
    ...
    e0 = mu_e - 3.0 * number_of_atoms * kB_T

    expected_sigma_e = np.sqrt((3.0 * number_of_atoms) / beta**2)  # in eV
    computed_sigma_e = np.std(relative_energies)
    std_relative_error = (expected_sigma_e - computed_sigma_e) / expected_sigma_e

    list_de = np.linspace(
        mu_e - 5 * computed_sigma_e, mu_e + 5 * computed_sigma_e, 1001
    )
    ...
    ax1.vlines(
        e0, ymin=0, ymax=1, color="k", linestyle="--", label=f"$E_0$ = {e0:5.3f} eV"
    )
    ax1.vlines(
        e0 + 3 * number_of_atoms * kB_T,
        ymin=0,
        ymax=1,
        color="r",
        linestyle="--",
        label=f"$E_0 + 3 N k_B T$ = {np.mean(relative_energies):5.3f} eV",
    )
    ...
    fig1.tight_layout()
    fig1.savefig(
        ANALYSIS_RESULTS_DIR.joinpath(f"{dataset_name}_distribution_analysis.png")
    )
    ...
    ax4.errorbar(
        list_kt, list_de, xerr=list_kt_err, yerr=list_de_err, fmt="o", label="Data"
    )

    list_kt_fit = np.linspace(0.01, 0.04, 101)
    list_de_ideal_fit = 3.0 * number_of_atoms * list_kt_fit
    ...
    fig2.tight_layout()
    fig2.savefig(
        ANALYSIS_RESULTS_DIR.joinpath(f"{dataset_name}_temperature_bins_analysis.png")
    )
    ...
    fig3.savefig(
        ANALYSIS_RESULTS_DIR.joinpath(
            f"{dataset_name}_energy_temperature_2D_histogram.png"
        )
    )
    ...
    ax6.hist(
        train_df["energy"].values, **common_params, color="red", label="Total Energy"
    )
    ...
    fig4.tight_layout()
    fig4.savefig(
        ANALYSIS_RESULTS_DIR.joinpath(f"{dataset_name}_potential_vs_total_energy.png")
    )
