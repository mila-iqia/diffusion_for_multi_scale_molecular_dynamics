...
prediction_head_parameters = MaceEquivariantScorePredictionHeadParameters(
    time_embedding_irreps="32x0e", gate="silu", number_of_layers=3
)

mace_score_network_parameters = MACEScoreNetworkParameters(
...
mlp_score_network_parameters = MLPScoreNetworkParameters(
    number_of_atoms=number_of_atoms, n_hidden_dimensions=3, hidden_dimensions_size=64
)
...
loss_monitoring_parameters = dict(
    number_of_bins=50, sample_every_n_epochs=1, spatial_dimension=spatial_dimension
)
...
if __name__ == "__main__":
    ...
    if number_of_atoms == 1:
        equilibrium_relative_coordinates = (
            0.5 * torch.ones(spatial_dimension)
        ).unsqueeze(0)
    ...
    diffusion_sampling_callback = HarmonicEnergyDiffusionSamplingCallback(
        noise_parameters,
        sampling_parameters,
        equilibrium_relative_coordinates,
        inverse_covariance,
    ...
    # Create a dataloader
    train_samples = get_exact_samples(
    ...
    train_energies = get_relative_harmonic_energy(
        train_samples, equilibrium_relative_coordinates, spring_constant
    )
    train_dataset = [
        {
    ...
    valid_samples = get_exact_samples(
        equilibrium_relative_coordinates, inverse_covariance, dataset_size
    )
    valid_energies = get_relative_harmonic_energy(
        valid_samples, equilibrium_relative_coordinates, spring_constant
    )
    ...
    callbacks_dict = create_all_callbacks(
        callback_parameters, output_directory=output_directory, verbose=True
    )
