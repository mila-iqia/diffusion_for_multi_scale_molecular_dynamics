...
class EnergyCalculator:
    ...
    def compute_oracle_energies(self, batch_relative_coordinates: torch.Tensor) -> np.ndarray:
        """Compute oracle energies."""
        batch_size = batch_relative_coordinates.shape[0]
        batched_unit_cells = einops.repeat(self.unit_cell, "d1 d2 -> b d1 d2", b=batch_size)

        batch_cartesian_positions = get_positions_from_coordinates(batch_relative_coordinates,
                                                                   batched_unit_cells)
...
if __name__ == '__main__':
    ...
    energy_calculator = EnergyCalculator(unit_cell=unit_cell, number_of_atoms=number_of_atoms)
    ...
    for number_of_corrector_steps, sigma_d, sigma_min, total_time_steps in tqdm(prod, "sample energies"):
        ...
        generated_samples_energies = energy_calculator.compute_oracle_energies(generated_samples)

        result = dict(sigma_d=sigma_d,
                      sigma_min=sigma_min,
