...
if __name__ == '__main__':
    ...
    inverse_covariance = spring_constant * torch.ones(number_of_atoms, spatial_dimension,
    ...
    fig.suptitle(f'Sigma normalized Score, 1 atom in 1D\n Equilibrium Position: ({x_eq}), '
                 f'Spring Constant = {spring_constant}, kmax = {kmax}')
    ...
    fig.tight_layout()
    fig.savefig(ANALYSIS_RESULTS_DIR / "analytical_score_network_1D.png")
    plt.close(fig)
    ...
    fig2.tight_layout()
    fig2.savefig(ANALYSIS_RESULTS_DIR / "analytical_score_network_1D_convergence.png")
    plt.close(fig2)
