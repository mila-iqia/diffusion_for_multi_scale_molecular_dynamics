...
class DiffusionSamplingParameters:
    ...
    """
    sampling_parameters: SamplingParameters  # Define the algorithm and parameters to draw samples.
    noise_parameters: NoiseParameters  # Noise for sampling, which can be different from training!
    metrics_parameters: SamplingMetricsParameters  # what should be done with the generated samples?


def load_diffusion_sampling_parameters(hyper_params: Dict[AnyStr, Any]) -> Union[DiffusionSamplingParameters, None]:
    ...
    assert 'sampling' in diffusion_sampling_dict, "The sampling parameters must be defined to draw samples."
    sampling_parameters = load_sampling_parameters(diffusion_sampling_dict['sampling'])

    assert 'noise' in diffusion_sampling_dict, "The noise parameters must be defined to draw samples."
    noise_parameters = NoiseParameters(**diffusion_sampling_dict['noise'])

    assert 'metrics' in diffusion_sampling_dict, "The metrics parameters must be defined to draw samples."
    metrics_parameters = SamplingMetricsParameters(**diffusion_sampling_dict['metrics'])

    return DiffusionSamplingParameters(sampling_parameters=sampling_parameters,
