...
from crystal_diffusion.models.score_networks.molegnn_score_network import (
    MolEGNNScoreNetwork, MolEGNNScoreNetworkParameters)
...
SCORE_NETWORK_PARAMETERS_BY_ARCH = dict(mlp=MLPScoreNetworkParameters,
                                        ...
                                        molegnn=MolEGNNScoreNetworkParameters)
...
def create_score_network(score_network_parameters: ScoreNetworkParameters):
    ...
    assert architecture in SCORE_NETWORKS_BY_ARCH.keys(), \
        f"Architecture {architecture} is not implemented. Possible choices are {SCORE_NETWORKS_BY_ARCH.keys()}"

    instantiated_score_network: ScoreNetwork = SCORE_NETWORKS_BY_ARCH[architecture](score_network_parameters)

    return instantiated_score_network


def create_score_network_parameters(score_network_dictionary: Dict[AnyStr, Any],
                                    global_parameters_dictionary: Dict[AnyStr, Any]) -> ScoreNetworkParameters:
    ...
    score_network_dataclass = SCORE_NETWORK_PARAMETERS_BY_ARCH[score_network_architecture]
    ...
    if 'prediction_head_parameters' in score_network_dictionary:
        ...
        prediction_head_parameters = (
            create_parameters_from_configuration_dictionary(configuration=head_config,
                                                            identifier="name",
                                                            options=MACE_PREDICTION_HEAD_BY_NAME))
        augmented_score_network_dictionary['prediction_head_parameters'] = prediction_head_parameters
    ...
    for key, value in augmented_score_network_dictionary.items():
        if key in global_parameters_dictionary:
            assert global_parameters_dictionary[key] == value, f"inconsistent configuration values for {key}"
    ...
    score_network_parameters = score_network_dataclass(**augmented_score_network_dictionary)
