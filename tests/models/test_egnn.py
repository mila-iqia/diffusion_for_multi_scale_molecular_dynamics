...
class TestEGNN:
    ...
    def edges(self, batch_size, number_of_atoms, num_edges):
        ...
        for b in range(batch_size):
            batch_edges = torch.Tensor([(i, j) for i in range(number_of_atoms) for j in range(number_of_atoms)])
    ...
    @pytest.fixture(scope='class')
    def batch(self, relative_coordinates, node_features, edges, batch_size, number_of_atoms):
        batch = {'coord': relative_coordinates.view(batch_size * number_of_atoms, -1),
    ...
    def generic_hyperparameters(self, node_features_size):
        hps = dict(input_size=node_features_size,
    ...
    def egcl_scores(self, batch, egcl, batch_size, number_of_atoms):
        ...
        return egcl_h.reshape(batch_size, number_of_atoms, -1), egcl_x.reshape(batch_size, number_of_atoms, -1)
    ...
    def permuted_node_features(self, batch_size, number_of_atoms, batch, permutations):
        ...
        permuted_h = torch.stack([h[batch_idx, permutations[batch_idx], :]
                                  for batch_idx in range(batch_size)])
    ...
    @pytest.fixture()
    def permuted_batch(self, permuted_coordinates, permuted_edges, permuted_node_features):
        permuted_batch = {'coord': permuted_coordinates,
    ...
    @pytest.fixture()
    def permuted_egnn_scores(self, permuted_batch, egnn, batch_size, number_of_atoms, spatial_dimension):
        egnn_scores = egnn(permuted_batch['node_features'], permuted_batch['edges'], permuted_batch['coord'])
        return egnn_scores.reshape(batch_size, number_of_atoms, spatial_dimension)

    @pytest.fixture()
    def permuted_egcl_scores(self, permuted_batch, egcl, batch_size, number_of_atoms):
        egcl_h, egcl_x = egcl(permuted_batch['node_features'], permuted_batch['edges'], permuted_batch['coord'])
        return egcl_h.reshape(batch_size, number_of_atoms, -1), egcl_x.reshape(batch_size, number_of_atoms, -1)

    def test_egcl_permutation_equivariance(self, egcl_scores, permuted_egcl_scores, batch_size, permutations):
        permuted_egcl_h, permuted_egcl_x = permuted_egcl_scores
        egcl_h, egcl_x = egcl_scores

        expected_permuted_h = torch.stack([egcl_h[batch_idx, permutations[batch_idx], :]
                                           for batch_idx in range(batch_size)])

        torch.testing.assert_close(expected_permuted_h, permuted_egcl_h)

        expected_permuted_x = torch.stack([egcl_x[batch_idx, permutations[batch_idx], :]
                                           for batch_idx in range(batch_size)])

        torch.testing.assert_close(expected_permuted_x, permuted_egcl_x)

    def test_egnn_permutation_equivariance(self, egnn_scores, permuted_egnn_scores, batch_size, permutations):
        expected_permuted_scores = torch.stack([egnn_scores[batch_idx, permutations[batch_idx], :]
                                                for batch_idx in range(batch_size)])
    ...
    def test_egcl_coord2radial(self, single_edge, fixed_distance, simple_pair_coord, egcl):
        computed_distance_squared, computed_displacement = egcl.coord2radial(single_edge, simple_pair_coord)
        torch.testing.assert_close(computed_distance_squared.item(), fixed_distance ** 2)
        torch.testing.assert_close(computed_displacement, simple_pair_coord[1, :].unsqueeze(0))
