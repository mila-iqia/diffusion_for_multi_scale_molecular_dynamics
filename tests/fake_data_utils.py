...
def generate_fake_configuration(
    ...
    cell_dimensions = 5.0 + 5.0 * np.random.rand(
        spatial_dimension
...
def generate_parse_dump_output_dataframe(
    ...
    for configuration in configurations:
        ...
        for coordinate_forces, name in zip(
            configuration.cartesian_forces.transpose(), ["fx", "fy", "fz"]
        ):
...
def create_dump_single_record(
    configuration: Configuration, timestep: int
) -> Dict[str, Any]:
...
def create_dump_yaml_documents(
    configurations: List[Configuration],
) -> List[Dict[str, Any]]:
    docs = []
...
def create_thermo_yaml_documents(
    configurations: List[Configuration],
) -> List[Dict[str, Any]]:
    ...
    for timestep, configuration in enumerate(configurations):
        row = (
            ...
            + [
                float(configuration.kinetic_energy),
                float(configuration.potential_energy),
            ]
...
def write_to_yaml(documents: List[Dict[str, Any]], output_file_path: str):
    ...
    with open(output_file_path, "w") as fd:
        ...
        yaml.dump_all(
            documents, fd, sort_keys=False, default_flow_style=None, width=1000
        )
...
def find_aligning_permutation(
    first_2d_array: torch.Tensor, second_2d_array: torch.Tensor, tol=1e-6
) -> torch.Tensor:
    ...
    distances = torch.sum(
        (first_2d_array[:, None, :] - second_2d_array[None, :, :]) ** 2, dim=-1
    )

    matching_indices = (distances < tol).nonzero()

    assert torch.allclose(
        matching_indices[:, 0], torch.arange(number_of_vectors)
